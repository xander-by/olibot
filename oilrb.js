...
const runcheck = async (chatid, text) => {
  ...
  const res = await fetch(
    `https://stats.datamark.by/api/label-card/?type_code=&series=${seria}&number=${number}`
  );
  ...
  if (json.IsFound === "1") {
    ...
    bot.sendMessage(
      chatid,
      `Информация по запросу <b>${textChek}</b> не найдена! Убедитесь что вы использовали английские буквы в серии!`,
...
const start = () => {
  ...
  bot.on("message", async (msg) => {
    ...
    if (msg.photo) {
      ...
      let photoLink = "";
      await bot.getFileLink(fileId).then((link) => {
        photoLink = link;
        ...
        if (labels.length) {
          ...
          if (array) {
            ...
            await bot.sendMessage(
              chatid,
              "Не обнаужили данных об акцизных марках на фото!"
            );
    ...
    if (text === "/start") {
      await bot.sendMessage(
        chatid,
        `Добро пожаловать в бот проверки акцизных марок! Выберите в меню нужную операцию`,
        { parse_mode: "HTML", disable_web_page_preview: true }
      );
      await bot.sendSticker(
        chatid,
        "https://tlgrm.ru/_/stickers/3b5/c3c/3b5c3c9e-3bc4-41c2-9cba-f288f3a4d5bb/4.webp"
      );
    ...
    if (text === "/info") {
      bot.sendMessage(
        chatid,
        `     Бот по проверке акцизных марок (моторного масла и прочих)\n Ваши пожелания, замечания, информацию об ошибках и т.д. пишите прямо в бот или высылайте по адресу: E-mail: <b>OilRbBot@gmail.com</b>\n Донаты и благодарности принимаются через <b>BTC кошелек 18i3LhkVfmEJ8MvaeQXMTjPbQxAPtXKJ2Z</b>`,
        { parse_mode: "HTML", disable_web_page_preview: true }
      );
      savehistory(chatid, text, "");
      return;
...
const promocodes = async (chatid) => {
  const Сообщ_ = `<b>Промокоды каршерингов Минска:</b>
...
const official = async (chatid) => {
  const Сообщ_ = `<b>    Официальные импортёры масла в РБ на 01.02.2019:</b>
